...
( function( $, api ) {
	...
	api.controlConstructor['soft-blog-repeater-text'] = api.Control.extend({
    ...
    // upsell 
    api.sectionConstructor['soft-blog-upsell'] = api.Section.extend( {
...
jQuery(document).ready(function($) {
    ...
    $('#soft-blog-img-container img').click(function() {      
        var item_id = $(this).parent().parent().parent().parent().attr('id');

        $('#' + item_id).find('li').each(function() {
            $('#' + item_id).find('img').removeClass('soft-blog-radio-img-selected') ;
        });

        $(this).addClass('soft-blog-radio-img-selected') ;
    });
